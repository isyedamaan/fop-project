...
public class Data {
    ...
    public int getJobsFinished(String startDateTime, String endDateTime){
        int count = 0;
        for(String[] line : jobsFinished){
            boolean timePeriod = line[0].compareTo(startDateTime)>=0 && line[0].compareTo(endDateTime)<0;
            if(timePeriod) 
                count++;
        }
    ...
    public int cleanUpsCompleted(String startDateTime, String endDateTime){
        int count = 0;
        for(String[] line : data){
            if(line.length<4) continue;
            boolean timePeriod = line[0].compareTo(startDateTime)>=0 && line[0].compareTo(endDateTime)<0;
            boolean condition = line[1].equals("cleanup_completing:");
            if(timePeriod && condition) 
                count++;
        }
        return count;
